...
class PrismaInfo {
  ...
  factory PrismaInfo.lookup(String excutable) {
    ...
    final entries = result.split('\n').where((element) => element.trim().isNotEmpty).map((e) => e.split(':')).where((element) => element.length == 2).map((e) => MapEntry(e[0], e[1])).map((e) => MapEntry(e.key.trim().toLowerCase(), e.value.trim()));
